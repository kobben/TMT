...
function deleteMap() {
  document.getElementById("mapDiv").children;
}

function createMap(guiState) {
  var VegaLiteFile = './vl/RoV.json';
  console.log("VegaLiteFile: " + VegaLiteFile);
    vegaEmbed("#mapDiv", VegaLiteFile, {"actions": false, "renderer": "svg"}).then(function(result) {
    }).catch(console.error);
}
